...
def RemoveEmptyTags(xmltags):
	for xmltag in xmltags.iter("*"):
		...
		if not (xmltag.text and xmltag.text.strip()):
			DLog("Removing empty XMLTag: " + xmltag.tag)
		...
		if nfoFile:
			...
			if nfoTextLower.count('<movie') > 0 and nfoTextLower.count('</movie>') > 0:
				...
				try: nfoXML = XML.ElementFromString(nfoText).xpath('//movie')[0]
					...
					DLog('ERROR: Cant parse XML in ' + nfoFile + '. Aborting!')
					return
				...
				try: 
					...
					Log("nfo value for <title> is :" + mytitle)
				except:
				...
				if mytitle != '':
					...
					Log("metadata.title NOW is set to : " + metadata.title)
				else:
					DLog("Title tag contained in nfo but it is EMPTY")
					pass
				...
				###### Sort Title    known as title_sort by Plex Plug-In framework 2
				DLog("======= SORT TITLE ========")
				try: DLog("Current 'metadata.title_sort' is : " + metadata.title_sort)
				except: pass
				try: DLog("current 'metadata.titleSort' is : " + metadata.titleSort)
				except: pass
				try: 
					mysorttitle = nfoXML.xpath('sorttitle')[0].text.strip()
					DLog("nfo value for sorttitle is : " + mysorttitle)
				except: 
					DLog("Unable to read XML tag 'sorttitle' - does not seem to be contained in nfo file")
					pass
				if mysorttitle != '':
					try:
						metadata.title_sort = mysorttitle
						DLog("metadata sort-title NOW was set to : " + metadata.title_sort)
					except:
						DLog("FAILED to adding sort-title '" + mysorttitle + "' to metadata of movie! Seems NOT supported")
				else:
					DLog("Sorttitle tag contained in nfo but it is EMPTY")
