	...
	def update(self, metadata, media, lang):
		...
		if nfoFile:
			...
			if nfoTextLower.count('<movie') > 0 and nfoTextLower.count('</movie>') > 0:    #contains the main tags --> lools like a KODI nfo file
				...
				#Log(nfoText)
				try: nfoXML = XML.ElementFromString(nfoText).xpath('//movie')[0]  #using absolute path in xpath()
				except:
				...
				#=========   WATCHED   =========
				Log("======= WATCHED ========")
				# Note: purposely only treating postitive path, i.e. setting Plex iten to watched, if it was not yet the case and XML claims it was watched
				# in theory, if XML has something as not watched that Plex already has marked as watched, the feed should set XML source to watched, but there is no link for that.
				thispath = inspect.getfile(inspect.currentframe())
				#Log("Script file path : " + thisfile)
				for i in range(1, 6):  #exits after i=5 - the hierarchy is fixed for Plugins (unless a major change happens) and the PMS folder should alwasy be 5 levels up from code.
					#Log("i is currently :  " + str(i))
					thispath = os.path.dirname(thispath)
					#Log(mypath)
					#thispath = mypath
				#myPMSpath = os.path.dirname(os.path.dirname(thisfile))
				mypath = thispath
				#Log("PMS file path : " + mypath)
				myprefpath = os.path.join(mypath, "Preferences.xml")
				Log("PMS Prefs file path : " + myprefpath)
				if os.path.exists(myprefpath):
					mypreftext = Core.storage.load(myprefpath)
					#Log(mypreftext)
				else:
					Log("Did NOT find Preferences file - please check Log. Aborting!")
					return
				mytoken = XML.ElementFromString(mypreftext).xpath('//Preferences/@PlexOnlineToken')[0]    #bit of a 'hack' but I got the token ;-)
				#Log(mytoken)
				#mytoken = "QBuAHqWHPpaxefxxsoSq"
				if Prefs['handlewatched']:
					myval = None
					try:
						myval= nfoXML.xpath('watched')[0].text    #.strip()
						Log("Value for <watched> in nfo-file is :  " + myval)
					except:
						Log("No <watched> tag in nfo-file : " + nfoFile)
						pass
					if myval == "true":
						# xml watched = true --> lets see what Plex has - only set if not yet set
						myheader = {"X-Plex-Token":mytoken}
						#myans = HTTP.Request("http://localhost:32400/library/metadata/11976?X-Plex-Token=" + mytoken)
						myans = HTTP.Request("http://localhost:32400/library/metadata/" + media.id, headers=myheader).content
						#Log(myans)
						try:
							myviewcount = XML.ElementFromString(myans).xpath('//MediaContainer/Video/@viewCount')[0]
							Log("XML attibute 'viewCount' is : " + str(myviewcount))
						except:
							Log("Arribute 'viewCount' not present in XML --> setting it to 0")
							myviewcount = 0
							pass
						#DLog("Plex server viewCount is : " + str(myviewcount))
						if myviewcount == 0:
							#now trying to set watched via URL request
							try:
								myurl = "http://localhost:32400/:/scrobble?identifier=com.plexapp.plugins.library&key=" + media.id + "&X-Plex-Token=" + mytoken
								myurl2 = "http://localhost:32400/:/scrobble?identifier=com.plexapp.plugins.library&key=" + media.id
								
								#Log("URL for HTTP.Request is : " + myurl)
								#myreply = 
								#HTTP.Precache(myurl)
								#myreply = 
								#myans = 
								HTTP.Request(myurl2, headers=myheader).content #headers
								#myans = HTTP.Request(myurl).headers #content #) #, cacheTime = 2) #,sleep=0.2)
								
								Log("Plex 'watched' status was just set to True (viewCount = 1)")
								#Log(myans)
							except:
								Log("Setting 'watched' via HTTP.Request failed")
								pass
							# try: 
								# Log(myans)
								# #replyxml = XML.ElementFromString(myans)
							# except:
								# Log("failed to assign variable 'replyxml' with XML-ElementFromString(myans) as 'myans' is empty")
								# pass
						else:
							Log("Plex server watched status was already set --> Nothing to do!")
					else: Log("watched either NOT present in nfo or set to false")  #not needed, but adds logging transparency
				...
				myval = None				
				try:
					myval= nfoXML.xpath("year")[0].text    #.strip()
					Log("Value for <year> in nfo-file is :" + myval)
				except:
					Log("No <year> tag in nfo-file : " + nfoFile)
					pass
				if myval:
					try:
						metadata.year = int(myval)
						Log("Plex item 'metadata.year' was just set to : " + str(metadata.year))
					except:
						Log("UNABLE to assing <year> to Plex item 'metadata.year'!")
						pass
				...
				try:
					myval= nfoXML.xpath("studio")[0].text    #.strip()
					Log("Value for <studio> in nfo-file is :" + myval)
				except:
					Log("No <studio> tag in nfo-file : " + nfoFile)
					pass
				if myval:
					try:
						metadata.studio = myval
						Log("Plex item 'metadata.studio' was just set to : " + metadata.studio)
					except:
						Log("UNABLE to assing <studio> to Plex item 'metadata.studio'!")
						pass
...
#myorigtitlestring =  nfoXML.xpath('originaltitle')[0].text.strip()
				...
				try: 
					...
					mytemp = nfoXML.xpath("set")[0].xpath("name")[0].text  # also works and is more flexible
				...
				metadata.collections.add(mytemp)   # or  mytemp.strip()
				...
				for mytag in nfoXML.xpath('tag'):
					...
					metadata.collections.add(mytagtxt)
